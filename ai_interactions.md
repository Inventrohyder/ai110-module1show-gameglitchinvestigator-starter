...
| AppTest simulation testing | `"BDD-style naming + AppTest simulation — No pytest-bdd Gherkin files. Use descriptive test_when_X_then_Y names. Use AppTest for e2e simulation of full game flows."`                                                                            | A test that boots up `app.py` via `AppTest.from_file`, enters guess, clicks button, and checks warning results. | No (initially failed/xfail)             | Ensured visual warning outputs are validated by running the real app.               |
